...
// Every PR I'm a reviewer on is listed — drafts, archived repos and ones another
// reviewer already sent back all stay in the list and get a badge instead. The
// queue is the complete picture; the badges are what drive triage.
...
function renderReview(){
  ...
  const note = degraded ? fetchErrNote('some review requests')
             : stale    ? staleNote('some review requests') : '';
...
document.getElementById('foot').textContent =
  'Every PR you are a reviewer on is listed, oldest-first — badges flag drafts, ' +
  'archived repos and PRs sitting with the author. Staleness is time since last ' +
  'update. Review copies a /code-review command. Regenerated by pr-dashboard/fetch.sh.';
